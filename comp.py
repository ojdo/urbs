...
def compare_scenarios(result_files, output_filename):
    ...
    created  =   esums.loc['Created'].rename(columns=lambda x:x.replace('.House','')).T
    consumed = - esums.loc['Consumed'].rename(columns=lambda x:x.replace('.House','')).T.drop('Demand', axis=1)
    ...
    sto_sums = sto_sums.rename(index=lambda x:x.replace('.House',''))
    ...
    sto_sums.columns = ['Battery', 'Tank']
    sto_sums = sto_sums.sort_index()
    sto_sums.loc[(slice(None), 'Electricity'), 'Tank'] = 0
